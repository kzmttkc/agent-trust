...
export const BLOG_POSTS: BlogPost[] = [
  {
    ...
    body: [
      ...
      "x402 is what finally gave that status code a job. It is a machine-payment protocol built directly on HTTP 402. The flow is deliberately small: a caller requests a paid endpoint, the server responds with 402 and a set of payment terms (an amount, a token, a destination, a chain), the caller pays on-chain, and then retries the same request with proof of that payment attached. On the second try the server sees the proof and returns the actual content. No account to create, no invoice to reconcile, no human clicking approve. The request and the payment travel in the same protocol that already moves everything else on the web.",
