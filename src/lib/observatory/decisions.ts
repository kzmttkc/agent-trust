...
export const DECISION_DEFINITION =
  "Each row is a decision the daily L1 runner actually made with real funds at stake, mapped 1:1 from the public ledger: refused_price_mismatch / refused_over_cap (wall demanded more than declared or over the hard cap — nothing signed), refused_wall_unpayable (no valid 402 / no machine-payable accept), paid_settled, paid_delivered_no_receipt, paid_no_settlement. Excluded as non-decisions: budget_denied, request_error, in_flight (vet402-side states).";
...
export async function getDecisionFeed(days: number, limit = 200): Promise<DecisionFeed> {
  ...
  const raw = await db.execute(sql`
    ...
    WHERE pu.attempted_at >= now() - make_interval(days => ${span}::int)
      AND pu.status IN ('price_mismatch','over_cap','no_402','no_eligible_accept','settled','delivered_no_receipt','settle_failed')
